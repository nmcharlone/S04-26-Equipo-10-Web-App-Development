...
interface EditReportModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: { estado: string; prioridad: string }) => void;
...
export default function EditReportModal({ open, onClose, onSubmit, initialEstado = "", initialPrioridad = "" }: EditReportModalProps) {
  ...
  useEffect(() => { setPrioridad(initialPrioridad); }, [initialPrioridad]);

  const canSubmit = !!estado && !!prioridad;

  const handleClose = () => { onClose(); };

  const handleSubmit = () => {
    if (!canSubmit) return;
    onSubmit({ estado, prioridad });
    handleClose();
  };
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 6 }}>Prioridad</label>
            <Dropdown value={prioridad} placeholder="Seleccionar prioridad" options={PRIORIDADES} onChange={setPrioridad} />
          </div>
        </div>
